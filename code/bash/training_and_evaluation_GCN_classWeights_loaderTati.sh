...
for i in "${CASOS[@]}"; do
    ...
    python3 ${PATH_ACTUAL}/code/python/training_and_evaluation_GCN_classWeights_loaderTati.py ${PATH_RDOS2} ${CLASE2} ${FLATTENED_NO} ${SYMMETRIC_ADJACENCY} ${PREPROC_ADJACENCY} ${PREPROC_FEATURES} ${i} |& tee ${PATH_RDOS2}/${BALANCED}_${i}a${NODOS}_NOflattened.txt &
    cp ${PATH_ACTUAL}/code/python/training_and_evaluation_GCN_classWeights_loaderTati.py ${PATH_RDOS2}/code_used/
    cp ${PATH_ACTUAL}/code/bash/training_and_evaluation_GCN_classWeights_loaderTati.sh ${PATH_RDOS2}/code_used/
done
