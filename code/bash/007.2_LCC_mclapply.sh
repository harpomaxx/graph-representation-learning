...
mkdir -p features/LCC_mclapply/time

for filename in ./*.ncol; do 
    base1=`basename ${filename} .ncol`
    base2=${base1%_*}
    Rscript ${PATH_REPO}/code/R/007.2_LCC_mclapply.R ${base2} > ${PATH_PWD}/features/LCC_mclapply/time/time_${base2}.txt
done    

mv *.csv features/LCC_mclapply/
