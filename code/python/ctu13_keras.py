...
#from tensorflow.keras.metrics import Precision, Recall, FalseNegatives, FalsePositives, TrueNegatives, TruePositives
...
#    # Calcula el peso de la clase positiva (minoritaria)
#    w = 0.1 / 0.9 # Ejemplo: clase positiva representa el 10% de las muestras
#
#    # Calcula la pérdida ponderada
#    loss = -(w * y_true * K.log(y_pred) + (1 - y_true) * K.log(1 - y_pred))
...
#        metrics=[precision, recall, fn, fp, tn, tp],
...
capture162 = pd.read_csv(
    "/content/drive/MyDrive/Colab Notebooks/capture20110816-2_noZeroB.ncol",
    sep=" ",  
...
capture162_features_tmp = pd.read_csv(
    "/content/drive/MyDrive/Colab Notebooks/capture20110816-2_features.csv",
    sep=",",  
    header=0
)
...
capture17 = pd.read_csv(
    "/content/drive/MyDrive/Colab Notebooks/capture20110817_noZeroB.ncol",
    sep=" ",  
    header=None,  # no heading row
    names=["source", "target", "weight"],  # set our own names for the columns
)
...
capture17_features_tmp = pd.read_csv(
    "/content/drive/MyDrive/Colab Notebooks/capture20110817_features.csv",
    sep=",",  
    header=0
)
...
edge_weights_ambos = edge_weights_ambos.reshape((148940,))
...
precision_result = history.history['precision'][-1]
recall_result = history.history['recall'][-1]
