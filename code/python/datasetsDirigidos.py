...
"""
  Script que contiene todos los datasets DIRIGIDOS generados usando 
...
######################################################################################################################################
###                                   Dirigido - 100 nodos - clases balanceadas                                                    ###
######################################################################################################################################
...
class synthetic_Dir_100nodes_balanced_clasesJunt(Dataset):
    """
    10 (ten) synthetic graphs:
      * directed (i.e. non-symmetric adjacency matrix)
      * 100 nodes each
    ...
    **Arguments**
    ...
    def download(self):
        ...
        for j in range(10):
            filename = os.path.join(self.path, f'graph_Dir_100nodes_balanced_clasesJunt_1{j}.npz')
    ...
    def read(self):
        ...
        for j in range(10):
            data = np.load(os.path.join(self.path, f'graph_Dir_100nodes_balanced_clasesJunt_1{j}.npz'), allow_pickle=True)
            if self.flattened:
                x_features = np.ones((n_nodes, n_features))
            else:
                x_features = data['x']
...
class synthetic_Dir_100nodes_balanced_clasesMezcl(Dataset):
    """
    10 (ten) synthetic graphs:
      * directed (i.e. non-symmetric adjacency matrix)
      * 100 nodes each
    ...
    **Arguments**
    ...
    def download(self):
        ...
        for j in range(10):
            filename = os.path.join(self.path, f'graph_Dir_100nodes_balanced_clasesMezcl_2{j}.npz')
    ...
    def read(self):
        ...
        for j in range(10):
            data = np.load(os.path.join(self.path, f'graph_Dir_100nodes_balanced_clasesMezcl_2{j}.npz'), allow_pickle=True)
            if self.flattened:
                x_features = np.ones((n_nodes, n_features))
            else:
                x_features = data['x']
...
######################################################################################################################################
###                               Dirigido - diferente cantidad de nodos - clases balanceadas                                      ###
######################################################################################################################################

    
class synthetic_Dir_diffSize_balanced_clasesSep(Dataset):
    """
    10 (ten) synthetic graphs:
      * directed (i.e. non-symmetric adjacency matrix)
    ...
    **Arguments**
    ...
    def download(self):
        ...
        for j in range(10):
            filename = os.path.join(self.path, f'graph_Dir_{n_nodes[j]}nodes_balanced_clasesSep_0{j}.npz')
            np.savez(filename, x=graphs1[j].x, a=graphs1[j].a, y=graphs1[j].y)
    ...
    def read(self):
        ...
        for j in range(10):
            data = np.load(os.path.join(self.path, f'graph_Dir_{n_nodes[j]}nodes_balanced_clasesSep_0{j}.npz'), allow_pickle=True)
            if self.flattened:
...
class synthetic_Dir_diffSize_balanced_clasesJunt(Dataset):
    """
    10 (ten) synthetic graphs:
      * directed (i.e. non-symmetric adjacency matrix)
      * number of nodes: 25,50,100,200,400,800,1600,3200,6400,12800
    ...
    **Arguments**
    ...
    def download(self):
        ...
        for j in range(10):
            filename = os.path.join(self.path, f'graph_Dir_{n_nodes[j]}nodes_balanced_clasesJunt_1{j}.npz')
            np.savez(filename, x=graphs2[j].x, a=graphs2[j].a, y=graphs2[j].y)
    ...
    def read(self):
        ...
        for j in range(10):
            data = np.load(os.path.join(self.path, f'graph_Dir_{n_nodes[j]}nodes_balanced_clasesJunt_1{j}.npz'), allow_pickle=True)
            if self.flattened:
                x_features = np.ones((n_nodes[j], n_features))
            else:
                x_features = data['x']
...
class synthetic_Dir_diffSize_balanced_clasesMezcl(Dataset):
    """
    10 (ten) synthetic graphs:
      * directed (i.e. non-symmetric adjacency matrix)
      * number of nodes: 25,50,100,200,400,800,1600,3200,6400,12800
    ...
    **Arguments**
    ...
    def download(self):
        ...
        for j in range(10):
            filename = os.path.join(self.path, f'graph_Dir_{n_nodes[j]}nodes_balanced_clasesMezcl_2{j}.npz')
            np.savez(filename, x=graphs3[j].x, a=graphs3[j].a, y=graphs3[j].y)
    ...
    def read(self):
        ...
        for j in range(10):
            data = np.load(os.path.join(self.path, f'graph_Dir_{n_nodes[j]}nodes_balanced_clasesMezcl_2{j}.npz'), allow_pickle=True)
            if self.flattened:
                x_features = np.ones((n_nodes[j], n_features))
            else:
                x_features = data['x']
...
######################################################################################################################################
###                                        Dirigido - 100 nodos - clases NO balanceadas                                            ###
######################################################################################################################################
...
class synthetic_Dir_100nodes_NoBalanced_1a100_clasesSep(Dataset):
    ...
    **Arguments**
    ...
    def read(self):
        ...
        for j in range(10):
            data = np.load(os.path.join(self.path, f'graph_Dir_100nodes_NoBalanced_1a100_clasesSep_0{j}.npz'), allow_pickle=True)
            if self.flattened:
                x_features = np.ones((n_nodes, n_features))
            else:
                x_features = data['x']
...
class synthetic_Dir_100nodes_NoBalanced_1a100_clasesMezcl(Dataset):
    ...
    **Arguments**
    ...
    def read(self):
        ...
        for j in range(10):
            data = np.load(os.path.join(self.path, f'graph_Dir_100nodes_NoBalanced_1a100_clasesMezcl_2{j}.npz'), allow_pickle=True)
            if self.flattened:
                x_features = np.ones((n_nodes, n_features))
            else:
                x_features = data['x']
...
class synthetic_Dir_100nodes_NoBalanced_2a100_clasesSep(Dataset):
    ...
    **Arguments**
    ...
    def read(self):
        ...
        for j in range(10):
            data = np.load(os.path.join(self.path, f'graph_Dir_100nodes_NoBalanced_2a100_clasesSep_0{j}.npz'), allow_pickle=True)
            if self.flattened:
                x_features = np.ones((n_nodes, n_features))
            else:
                x_features = data['x']
...
class synthetic_Dir_100nodes_NoBalanced_2a100_clasesMezcl(Dataset):
    ...
    **Arguments**
    ...
    def read(self):
        ...
        for j in range(10):
            data = np.load(os.path.join(self.path, f'graph_Dir_100nodes_NoBalanced_2a100_clasesMezcl_2{j}.npz'), allow_pickle=True)
            if self.flattened:
                x_features = np.ones((n_nodes, n_features))
            else:
                x_features = data['x']
...
class synthetic_Dir_100nodes_NoBalanced_5a100_clasesSep(Dataset):
    ...
    **Arguments**
    ...
    def read(self):
        ...
        for j in range(10):
            data = np.load(os.path.join(self.path, f'graph_Dir_100nodes_NoBalanced_5a100_clasesSep_0{j}.npz'), allow_pickle=True)
            if self.flattened:
                x_features = np.ones((n_nodes, n_features))
            else:
                x_features = data['x']
...
class synthetic_Dir_100nodes_NoBalanced_5a100_clasesMezcl(Dataset):
    ...
    **Arguments**
    ...
    def read(self):
        ...
        for j in range(10):
            data = np.load(os.path.join(self.path, f'graph_Dir_100nodes_NoBalanced_5a100_clasesMezcl_2{j}.npz'), allow_pickle=True)
            if self.flattened:
                x_features = np.ones((n_nodes, n_features))
            else:
                x_features = data['x']
...
class synthetic_Dir_100nodes_NoBalanced_10a100_clasesSep(Dataset):
    ...
    **Arguments**
    ...
    def read(self):
        ...
        for j in range(10):
            data = np.load(os.path.join(self.path, f'graph_Dir_100nodes_NoBalanced_10a100_clasesSep_0{j}.npz'), allow_pickle=True)
            if self.flattened:
                x_features = np.ones((n_nodes, n_features))
            else:
                x_features = data['x']
...
class synthetic_Dir_100nodes_NoBalanced_10a100_clasesMezcl(Dataset):
    ...
    **Arguments**
    ...
    def read(self):
        ...
        for j in range(10):
            data = np.load(os.path.join(self.path, f'graph_Dir_100nodes_NoBalanced_10a100_clasesMezcl_2{j}.npz'), allow_pickle=True)
            if self.flattened:
                x_features = np.ones((n_nodes, n_features))
            else:
                x_features = data['x']
...
class synthetic_Dir_100nodes_NoBalanced_20a100_clasesSep(Dataset):
    ...
    **Arguments**
    ...
    def read(self):
        ...
        for j in range(10):
            data = np.load(os.path.join(self.path, f'graph_Dir_100nodes_NoBalanced_20a100_clasesSep_0{j}.npz'), allow_pickle=True)
            if self.flattened:
                x_features = np.ones((n_nodes, n_features))
            else:
                x_features = data['x']
...
class synthetic_Dir_100nodes_NoBalanced_20a100_clasesMezcl(Dataset):
    ...
    **Arguments**
    ...
    def read(self):
        ...
        for j in range(10):
            data = np.load(os.path.join(self.path, f'graph_Dir_100nodes_NoBalanced_20a100_clasesMezcl_2{j}.npz'), allow_pickle=True)
            if self.flattened:
                x_features = np.ones((n_nodes, n_features))
            else:
                x_features = data['x']
...
class synthetic_Dir_diffSize_NoBalanced_1percent_clasesSep(Dataset):
    ...
    **Arguments**
    ...
    def read(self):
        ...
        for j in range(10):
            data = np.load(os.path.join(self.path, f'graph_Dir_NoBalanced_{n_infected[j]}a{n_nodes[j]}_clasesSep_0{j}.npz'), allow_pickle=True)
            if self.flattened:
                x_features = np.ones((n_nodes[j], n_features))
            else:
                x_features = data['x']
...
class synthetic_Dir_diffSize_NoBalanced_1percent_clasesMezcl(Dataset):
    ...
    **Arguments**
    ...
    def read(self):
        ...
        for j in range(10):
            data = np.load(os.path.join(self.path, f'graph_Dir_NoBalanced_{n_infected[j]}a{n_nodes[j]}_clasesMezcl_2{j}.npz'), allow_pickle=True)
            if self.flattened:
                x_features = np.ones((n_nodes[j], n_features))
            else:
                x_features = data['x']
