...
df <- readr::read_csv("all_captures_except_9_pkts.csv", show_col_types = FALSE)
...
for (cluster in numberClusters) {
    print(cluster)
    k <- append(k, cluster)
    zz <- file(paste("kmeans_", cluster, "_clusters_",TIPO,"_normalizado.txt", sep = ""), open = "wt")
    sink(zz)
    ...
    cat("\n For k = ", cluster, ": \n")
    #gc()
    time_kmeans <- system.time(kmeans.re <- kmeans(X, centers = cluster, nstart = 10, iter.max=1000, algorithm = TIPO))
...
readr::write_csv(df, paste("features_normalized_and_kmeans_",TIPO,"_normalizado.csv",sep="")) 

hobob <- data.frame(k,HOB,HOB_percent,BOB,BOB_percent)
readr::write_csv(hobob, paste("HOB_BOB_table_",TIPO,"_normalizado.csv",sep=""))
