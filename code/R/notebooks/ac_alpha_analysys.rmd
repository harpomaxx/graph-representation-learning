...
kmeans_data_all <- list()
for (alpha in p$calculate_ac$alphas) {
  kmeans_data<-yaml::read_yaml(paste0("metrics/kmeans/asis/kmeans_alpha_",alpha,"_r_.yaml"))
   kmeans_data<-kmeans_data |> as.data.frame()
  kmeans_data$alpha<-alpha
  kmeans_data_all<-rbind(kmeans_data_all,kmeans_data)
...
kmeans_data_all %>% mutate(alpha=as.factor(alpha)) %>% filter(alpha=="0.01") %>% knitr::render_markdown()
```