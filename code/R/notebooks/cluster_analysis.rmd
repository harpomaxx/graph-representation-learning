...
#features_data %>% readr::write_csv("./all_but_capture9-lcc-non-directed.csv")
...
```
```{r}
features_data <- readr::read_csv("all_captures_except_9_SINnormalizar_pkts.csv")
...
selected_features <- c("ID","OD","IDW","ODW","BC","AC","LCC")
...
# Clustering
```{r}
cluster_data <- kmeans(features_data %>% select(any_of(selected_features)),centers = 25,nstart = 10, iter.max = 10000)
features_cluster<-data.frame(features_data,cluster=cluster_data$cluster)
gc()
```
...
features_cluster %>% filter(cluster == begnin_cluster) %>% group_by(label) %>% count()
```
```{r}
clusters_size <- features_cluster %>% group_by(cluster) %>% count()
begnin_cluster<-clusters_size  %>% arrange(desc(n)) %>% head(1) %>% select(cluster) %>% unlist() %>% unname()
