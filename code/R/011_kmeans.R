...
for (cluster in numberClusters) {
    print(cluster)
    k <- append(k, cluster)
    zz <- file(paste("kmeans_", cluster, "_clusters_",TIPO,"_sinAC.txt", sep = ""), open = "wt")
    sink(zz)
...
readr::write_csv(df, paste("features_normalized_and_kmeans_",TIPO,"_sinAC.csv",sep="")) 

hobob <- data.frame(k,HOB,HOB_percent,BOB,BOB_percent)
readr::write_csv(hobob, paste("HOB_BOB_table_",TIPO,"_sinAC.csv",sep=""))
